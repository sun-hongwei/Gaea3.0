...
function defaultClickEvent(e) {
    ...
    try {
        ...
        if (!Tools.isNull(allowId)){
            var jumpData = Tools.dynamicCallFunctionByName(uiData.workflow.name + "_allowJump", [uiData, data, allowId]);
			if (!Tools.isNull(jumpData)){
				if (!Tools.isNull(jumpData.allow) && !jumpData.allow){
					return;
				}
				
				jumpParams[jumpData.id] = jumpData.data;
			}
        }
