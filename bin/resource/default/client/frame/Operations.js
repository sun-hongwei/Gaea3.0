...
function internalGetFrameControlByName(uiInfo, name) {
    ...
    var control = internalGetFrameControlByNameForDiv(formDiv, name);
    if (Tools.isNull(control)) {
        traversePopupDiv(function (index, element) {
            var childs = $(element).find("[name='" + name + "']");
            if (childs.length == 0)
                childs = $(element).find("#" + name);
            if (childs.length > 0) {
                control = mini.getByName(name, element);
                if (Tools.isNull(control))
                    control = childs[0];
...
function internalGetFrameControlByNameForDiv(div, name) {
...
function traversePopupDiv(eachProc) {
    var findObj = $(document.body).children(".mini-popup");
    if (!Tools.isNull(findObj))
        findObj = findObj.find("div");
    if (!Tools.isNull(findObj)) {
        for (let index = 0; index < findObj.length; index++) {
            const element = findObj[index];
            var isok = eachProc(index, element);
            if (!Tools.isNull(isok) && !isok)
                return;
        }
    }
}
...
function initDivDataForUIInfo(uiInfo, element, elementInfo) {
    ...
    if (Tools.isNull(div)) {
        traversePopupDiv(function (index, element) {
            if (elementInfo.element.data.name == element.name) {
                div = element;
                return false;
            }
        });
...
function reInitUIInfoFrameControlHash(uiInfo) {
    uiInfo.nameHash = undefined;
    initUIInfoFrameControlHash(uiInfo);
}
/**
...
function initUIInfoFrameControlHash(uiInfo) {
    if (Tools.isNull(uiInfo.nameHash)) {
        ...
        uiInfo.data.forEach(function (element) {
            ...
            var typename = element.data.typename;
            if (Tools.isNull(typename)) {
                typename = element.data.type;
            }
            switch (typename) {
                case cc.Report_Name:
